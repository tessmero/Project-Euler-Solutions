 ...
 * problem, which is used for quickly testing. (See {@link
 * com.tessmero.projecteuler.solvers.Solver#getExpectedTestResult()})
 *
...
public class FastSolverTest extends BaseTest {
  ...
  /**
   * For all Solver implementations: create an instance, call {@link Solver#doTest}, and assert that
   * the return value matches {@link Solver#getExpectedTestResult}.
   */
  @Test
  public void testAllSolvers() throws Exception {
    ...
    for (int problemNumber : allSolvers.keySet()) {
      ...
      assertEquals(
          format("unexpected quick-test result from {0} ({1})", solverShortName, problemUrl),
          expResult,
          result);
