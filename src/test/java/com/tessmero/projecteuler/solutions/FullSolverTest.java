...
public class FullSolverTest extends BaseTest {
  ...
  public void testAllSolvers() throws Exception {
    ...
    for (int problemNumber : allSolvers.keySet()) {
      ...
      assertEquals(
          format(
              "Solver returned incorrect solution for problem #{0} ({1})",
              problemNumber, problemUrl),
          expectedSolution,
          solution);
      ...
      logger.info(
          format(
              "got correct solution ({0}) from {1}: {2}", solution, solverShortName, problemUrl));
    }
  }
    ...
    try (BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream()))) {
      ...
      while ((line = in.readLine()) != null) {
        try {
          ...
          throw new Exception(
              format(
                  "Exception parsing problem number and correct solution from line \"{0}\"", line),
              e);
        }
      }
    }
