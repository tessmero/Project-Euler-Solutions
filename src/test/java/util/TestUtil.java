...
/**
 * Provides a static convenience method for listing all implementations of {@link
 * com.tessmero.projecteuler.solvers.Solver} present in the source.
 *
 * @author Oliver
 */
public class TestUtil {
  ...
  /**
   * Get a list of instances for each implementation of {@link
   * com.tessmero.projecteuler.solvers.Solver} present in the source.
   *
   ...
   *
   * @return a map where keys are problem numbers and values are {@link
   *     com.tessmero.projecteuler.solvers.Solver} instances
                ...
                toMap(
                    ...
                    (v1, v2) -> {
                      throw new RuntimeException(
                          format("Duplicate key for solvers {0} and {1}", v1, v2));
                    },
                    TreeMap::new));
  ...
  private static File getSolverSourceDir() {
    ...
    if (!srcDir.isDirectory()) {
      throw new Error(
          format(
              "could not find solvers source directory at path \"{0}\"", srcDir.getAbsolutePath()));
    }
    return srcDir;
  }
  ...
  private static void assertSolverSourceFilenameConventions(String srcFilename) {
    if (!(srcFilename.startsWith("Solver") && srcFilename.endsWith(".java"))) {
      fail(
          format(
              "unexpected filename in solutions source directory"
                  + "\n\tdirectory: {0}\n\tfilename: {1}",
              solverSourceDirPath, srcFilename));
    }
  }
  ...
  private static Solver getSolverInstance(String srcFilename) {
    try {
      ...
      fail(
          format(
              "Malformed solver source file \"{0}\". Could not create an instance:\n{1}",
              srcFilename, e));
      return null;
    }
  }
