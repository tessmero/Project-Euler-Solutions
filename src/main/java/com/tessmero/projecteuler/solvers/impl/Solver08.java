...
public class Solver08 extends LongSolver {
  ...
  private long getLargestProduct(int numAdjacentDigits) {
    long result = -1;
    for (int startIndex = 0; startIndex + numAdjacentDigits < 1000; startIndex++) {
      long product = 1;
      for (int dindex = 0; dindex < numAdjacentDigits; dindex++) {
        product *= thousandDigits.charAt(startIndex + dindex) - 48;
      }
      if (product > result) {
        result = product;
      }
