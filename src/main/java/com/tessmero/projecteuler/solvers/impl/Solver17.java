...
public class Solver17 extends LongSolver{
  ...
  String[] tensWords = { 
    "","{0}teen","twenty-{0}","thirty-{0}","forty-{0}","fifty-{0}",
  ...
  List<Integer> ignorableChars = unmodifiableList( " -".chars()
          .mapToObj( ch -> ch ).collect( toList() ) );
  
  private long countLetters( String word ) {
    return word.chars()
            .filter( ch -> !ignorableChars.contains( ch ) )
            .count();
  }
