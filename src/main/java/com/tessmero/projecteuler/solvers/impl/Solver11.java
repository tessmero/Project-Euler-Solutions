...
public class Solver11 extends LongSolver {
  ...
  public long doTest() throws Exception {
    return getProduct( getFactors( 8, 6, Dir.Diagonal ) );
  }
  ...
  private long[] getFactors( int col, int row, Dir dir ) throws Exception {
    ...
    for ( int i = 1 ; i < 4 ; i++ ) {
      if ( dir == Dir.Vertical || dir == Dir.Diagonal || dir == Dir.BackDiagonal ) {
        row++;
      }
      if ( dir == Dir.Horizontal || dir == Dir.Diagonal ) {
        col++;
      }
      if ( dir == Dir.BackDiagonal ) {
        col--;
      }
      try {
  ...
  private enum Dir{
    Vertical, Horizontal, Diagonal, BackDiagonal
  }
}
