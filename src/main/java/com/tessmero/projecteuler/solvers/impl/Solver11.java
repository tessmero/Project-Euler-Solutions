...
public class Solver11 extends LongSolver {
                    ...
                    range(0, 20)
                        .flatMap(
                            row ->
                                ...
                                stream(Directions).map(dir -> getProduct(col, row, dir))))

            //get the maximum product from all combinations above
            .max()
            .getAsLong();
  ...
  public long doTest() throws Exception {
    return getProduct(8, 6, Diagonal);
  }
  ...
  private LongStream getNumbersFromGrid(long col, long row, long dir) {
    ...
    for (int i = 1; i < 4; i++) {
      if (dir == Vertical || dir == Diagonal || dir == BackDiagonal) {
        row++;
      }
